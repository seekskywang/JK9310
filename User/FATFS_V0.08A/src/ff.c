...
//	vu16 boot_sig;
//	vu32 boot_sig1;
//	//MS_BulkRecv(0, 1, fs->win);
//	if (disk_read(fs->drv, fs->win, sect, 1) != RES_OK)	/* Load boot record */
//		return 3;
//	boot_sig = ReadLE16U(&FATBuffer[510]);
//	if (boot_sig != 0xAA55)		/* Check record signature (always placed at offset 510 even if the sector size is >512) */
//		return 2;
//		boot_sig1=LD_DWORD(&FATBuffer[BS_FilSysType] ) ;
//	if ((boot_sig1 & 0xFFFFFF) == 0x544146)	/* Check "FAT" string */
//		return 0;
...
static
BYTE check_fs (	/* 0:The FAT BR, 1:Valid BR but not an FAT, 2:Not a BR, 3:Disk error */
volatile	FATFS *fs,	/* File system object */
	DWORD sect	/* Sector# (lba) to check if it is an FAT boot record or not */
)
{
	if (disk_read(fs->drv, ( BYTE *)fs->win, sect, 1) != RES_OK)	/* Load boot record */
	{
		//&fs->win[0]=FATBuffer;
		return 3;
		
	}
	if (ReadLE16U(&fs->win[510]) != 0xAA55)		/* Check record signature (always placed at offset 510 even if the sector size is >512) */
		return 2;
