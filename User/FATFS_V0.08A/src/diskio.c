...
//    Status = MSD_Init();
    Status = MS_Init( &blkSize, &numBlks, inquiryResult );
    if(Status == 0)
	  return 0;
    else 
	  return STA_NOINIT;
...
)
{
  ...
  switch (drv)
  {
    ...
    {       
	  Status =  MS_BulkRecv(sector, 1,( void*) FATBuffer);
		memcpy(buff, (void *)FATBuffer, count*MS_BlkSize);
...
)
{
  ...
  switch (drv)
  {
    ...
    {   
		memcpy((void *)FATBuffer,buff, count*MS_BlkSize);
		//buff=(const BYTE *)FATBuffer;
		//FATBuffer=(volatile uint8_t *)buff;
	  Status =  MS_BulkSend(sector, 1, (void*)FATBuffer);
    }                                                
    else                    /* 多个sector的写操作 */    
    {   
