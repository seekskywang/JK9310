...
//  LPC_IOCON->P2_0   = 0x27;  // LCD_PWR
	...
	/*Back light enable*/
	LPC_GPIO1->DIR |= (1<<1);//p1.2
	LPC_GPIO1->SET|= (2<<1);
	
	LPC_GPIO0->DIR |= (1<<1);//p2.1
	LPC_GPIO0->SET|= (21<<1);
...
void LCD_DrawRect_Rect( uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1 , uint32_t color )
{
//	unsigned long i,j;
...
{
	unsigned long i,j;
	uint16_t *pDst= (uint16_t *)LCD_VRAM_BASE_ADDR + (y0)*480+x0;
...
void LCD_DrawCircle(uint16_t Xpos, uint16_t Ypos, uint16_t Radius,uint32_t color)
{
    ...
    do {
		...
		*(pDst + ((Xpos-x) + LCD_PIXEL_WIDTH*(Ypos+y)))= color;       //GB
...
void LCD429_PutPixel(uint16_t Xpos,uint16_t Ypos,uint32_t color)
{
...
void PutChar_12( uint16_t Xpos, uint16_t Ypos, uint8_t ASCI, uint16_t charColor, uint16_t bkColor )
{
	uint32_t i, j;
...
void lcd_display_coord(uint16_t x,uint16_t y,uint16_t color,uint16_t color1)
{
    uint16_t dx,dy;
    uint16_t dx1,dy1;
    uint16_t i;
...
//		PutChar( x0, y0, *pcStr, Colour.Fword, Colour.black );  
		...
		//LCD_ShowFontCN_40_55(x0+usWidth,y0,24,51,(uint8_t*)(_num2)+i*24/8*51);	
...
void lcd_image(uint8_t *pt)
{
    ...
    for( i = 0; (C_GLCD_H_SIZE * C_GLCD_V_SIZE) > i; i++)
    {
        ...
        color=(uint32_t)*(pt+2)|(uint32_t)*(pt+1)<<8|(uint32_t)*(pt)<<16;
    	*pDst++ = color; 
		*pt++;
		*pt++;
		*pt++;
...
//    }
//}
//void disp_mage(uint16_t x,uint16_t y,uint16_t xsize,uint16_t ysize,uint32_t color)
//{
//void LCD_ShowFontCN_40_55(uint16_t x,uint16_t y,uint16_t xsize,uint16_t ysize,uint8_t * image)
//{                                  
//	uint8_t temp,t1;
